"""
Модели случайного леса на синтетических проектных метриках.

Одна строка CSV = один проект. Скрипт проверяет три проектных таргета:
1) центральную длительность проекта;
2) абсолютную ширину хвоста P90-P50;
3) относительную ширину хвоста Schedule Risk Ratio = (P90 - P50) / P50.

Для каждого таргета сравниваются несколько профилей RF. Выбор модели делается
по cross-validation на обучающей части, а итоговые метрики считаются на holdout.
"""
...
RANDOM_STATE = 42
VIS_DIR = figures("rf_synth_project")
...
TARGETS = [
    ("det_duration_days", "длительность проекта", "дни", "duration"),
    ("p90_minus_p50", "абсолютная ширина хвоста P90-P50", "дни", "tail_width"),
    ("schedule_risk_ratio", "Schedule Risk Ratio", "доля", "risk_ratio"),
]
...
def bool_series(series: pd.Series) -> pd.Series:
    """Надёжно приводит CSV-колонку True/False к bool."""
...
def train_target(df: pd.DataFrame, target_col: str, target_label: str, unit: str, slug: str) -> dict:
    ...
    print(f"\n─── Цель: {target_label} ───")
    print(f"Среднее значение:  {y.mean():.3f} {unit}")
    print(f"Медиана:           {y.median():.3f} {unit}")
    print(f"Обучающая выборка: {len(X_train)} проектов")
    print(f"Тестовая выборка:  {len(X_test)} проектов")
    ...
    print(f"\nВыбрана модель по CV: {selected_name}")
    ...
    print("\nВажность признаков выбранной модели:")
...
def save_plots(
    ...
    ax.set_xlabel(f"Фактическое значение ({unit})")
    ax.set_ylabel(f"Предсказанное значение ({unit})")
    ax.set_title(f"Факт vs предсказание: {target_label}")
    fig.tight_layout()
    ...
    ax.set_xlabel(f"Ошибка ({unit})")
    ax.set_title(f"Распределение ошибок: {target_label}")
    fig.tight_layout()
    ...
    ax.set_xlabel("Вклад")
    ax.set_title(f"Важность признаков: {target_label}")
    fig.tight_layout()
