"""
Synthetic dataset generator for LTRROE.
Generates random projects with employees, tasks, and dependencies
and computes actual task durations with the core algorithms.
"""
...
NUM_PROJECTS = 1000              # number of generated projects
MIN_TASKS = 5
MAX_TASKS = 30
MIN_EMPLOYEES = 3
MAX_EMPLOYEES = 15
...
def random_skills(max_skills=4):
    """Return a random subset of skills with size 1..max_skills."""
    return random.sample(SKILL_POOL, k=random.randint(1, max_skills))

def random_efficiency(skills):
    """Generate a skill-efficiency dictionary for each skill, from 0.6 to 1.4."""
    return {skill: round(random.uniform(0.6, 1.4), 2) for skill in skills}

def random_triple(low=1, high=30):
    """Generate an optimistic/likely/pessimistic triplet with low <= likely <= high."""
...
def generate_dependencies(task_ids, density=0.3):
    """
    Generate random forward-only dependencies to avoid cycles
    """
    deps = []
...
def generate_project(proj_id):
    ...
    Return a list of feature dictionaries, one per task
    """
    project = Project()
    ...
    # Calculate the schedule and actual durations
    _, _, task_duration = calculate_schedule(project)

    # Build features for each task
    task_data = []
...
if __name__ == "__main__":
    ...
    print(f"Dataset saved: {OUTPUT_CSV} ({len(df)} tasks of {NUM_PROJECTS} projects).")
