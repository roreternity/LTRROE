"""
Project-level synthetic dataset generator for LTRROE.

The output CSV is compatible with the main columns of metrics_results_full.csv:
one row represents one project, not one task.
"""
...
def random_triple(low=1.0, high=18.0):
    """PERT triplet in days: optimistic <= likely <= pessimistic."""
...
def generate_dependencies(task_ids, min_dependencies=MIN_DEPENDENCIES):
    """Random DAG: dependencies only go from lower task_id to higher task_id."""
    possible_edges = [
        (from_id, to_id)
...
def build_dataset(num_projects, num_simulations, output_csv):
    ...
    while len(rows) < num_projects:
        ...
        rows.append(project_to_metrics(project, num_simulations))
        if len(rows) % 100 == 0 or len(rows) == num_projects:
            ok = sum(1 for row in rows if row["mc_success"])
            print(f"  [{len(rows)}/{num_projects}] success={ok}")
    ...
    print(f"\nSaved: {output_csv}")
    print(f"Total: {len(rows)} projects, successful MC: {len(ok_rows)}")
    if ok_rows:
        print(
            "Schedule risk ratio: "
