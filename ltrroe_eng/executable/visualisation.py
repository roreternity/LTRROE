...
def plot_monte_carlo_histogram(project_durations: List[float], deadline: int = 35) -> Tuple[Optional[plt.Figure], Optional[plt.Axes]]:
    """
    ...
    try:
        ...
        prob_success = (np.array(project_durations) <= deadline).mean() * 100
        
        # Text block with statistics
        stats_text = (
            f"Statistics:\n"
            f"Iterations: {len(project_durations)}\n"
            f"Mean: {mean_duration:.1f} days\n"
            f"Median: {median_duration:.1f} days\n"
            f"Min: {np.min(project_durations):.1f} days\n"
            f"Max: {np.max(project_durations):.1f} days\n"
            f"Success probability: {prob_success:.1f}%"
        )
        ...
        ax.set_ylabel('Frequency (number of iterations)')
        ax.legend()
