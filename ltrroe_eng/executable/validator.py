"""
LTRROE System Validator
Validates project data integrity and consistency across all entities
Ensures data quality for simulation and analysis modules
"""
...
print(len(project.proj_tasks))

# Problem severity levels
class ValidationLevel(Enum):
    ERROR = "ERROR"      # critical, must be fixed
    WARNING = "WARNING"  # not critical, but suspicious
    INFO = "INFO"        # informational only

# Structure for a single found issue
@dataclass
class ValidationIssue:
    level: ValidationLevel  # level (ERROR/WARNING/INFO)
    message: str           # description of the problem
    object_type: str       # 'Employee', 'Task', 'Dependency', 'Outsource', 'Project'
    object_id: Any         # object ID (emp_id, task_id, dep_id, etc.)
    field: str = None      # which field has the problem
    value: Any = None      # problematic value

class LTRROEValidator:
    def __init__(self):
        self.issues = []  # all found problems will be stored here
    
    def validate_project(self, project) -> dict:
        """
        Main method: validates the entire project.
        
        Returns:
            Dictionary with validation results
        """
        self.issues.clear()  # clear old results
        
        # Sequentially validate all project parts
        ...
        # self._validate_assignments(project)  # Skipping for now
    ...
    def _check_project_basics(self, project):
        ...
        # Check for employees dictionary
        if not hasattr(project, 'proj_employees'):
            self._add_issue(ValidationLevel.ERROR,
                          "Project does not contain employees dictionary",
        ...
        # Check for dependencies dictionary
        if not hasattr(project, 'proj_dependencies'):
            self._add_issue(ValidationLevel.ERROR,
                          "Project does not contain dependencies dictionary",
    ...
    def _validate_employee(self, employee, emp_id):
        ...
        if hasattr(employee, 'emp_name'):
            if not employee.emp_name:
                ...
                self._add_issue(ValidationLevel.ERROR, 
                              "Employee name consists only of whitespace",
                              'Employee', emp_id, 
                              'emp_name', employee.emp_name)
        ...
        if hasattr(employee, 'emp_skills'):
            if not isinstance(employee.emp_skills, list):
                ...
                self._add_issue(ValidationLevel.WARNING, 
                              "Employee has no specified skills",
                              'Employee', emp_id, 
        ...
        if hasattr(employee, 'emp_error_prob'):
            if not isinstance(employee.emp_error_prob, (int, float)):
                ...
                self._add_issue(ValidationLevel.ERROR,
                              "Error probability must be in range 0.0-1.0",
                              'Employee', emp_id,
                              'emp_error_prob', employee.emp_error_prob)
        ...
        if hasattr(employee, 'emp_efficiency'):
            if not isinstance(employee.emp_efficiency, dict):
                self._add_issue(ValidationLevel.ERROR, 
                              "Efficiency must be a dictionary {skill: efficiency}", 
                              'Employee', emp_id, 
                ...
                for skill, efficiency in employee.emp_efficiency.items():
                    if not isinstance(efficiency, (int, float)):
                        self._add_issue(ValidationLevel.ERROR,
                                      f"Skill '{skill}' efficiency must be a number",
                                      'Employee', emp_id,
                                      f'emp_efficiency[{skill}]', efficiency)
                    elif not 0.0 <= efficiency <= 10.0:
                        self._add_issue(ValidationLevel.ERROR,
                                      f"Skill '{skill}' efficiency must be in range 0.0-10.0",
                                      'Employee', emp_id,
                                      f'emp_efficiency[{skill}]', efficiency)
        ...
        if hasattr(employee, 'emp_current_load'):
            if not isinstance(employee.emp_current_load, (int, float)):
                self._add_issue(ValidationLevel.ERROR,
                              "Current load must be a number",
                              'Employee', emp_id,
                              'emp_current_load', employee.emp_current_load)
            elif employee.emp_current_load < 0:
                self._add_issue(ValidationLevel.WARNING,
                              "Current load is negative",
                              'Employee', emp_id,
                              'emp_current_load', employee.emp_current_load)
            
            # Check for exceeding maximum load
            if hasattr(employee, 'emp_max_daily_hours'):
                if employee.emp_current_load > employee.emp_max_daily_hours:
                    self._add_issue(ValidationLevel.WARNING, 
                                  f"Current load ({employee.emp_current_load}) exceeds maximum ({employee.emp_max_daily_hours})",
                                  'Employee', emp_id, 
                                  'emp_current_load', employee.emp_current_load)
        ...
        if hasattr(employee, 'emp_fatigue'):
            if not isinstance(employee.emp_fatigue, (int, float)):
                self._add_issue(ValidationLevel.ERROR,
                              "Fatigue multiplier must be a number",
                              'Employee', emp_id,
                              'emp_fatigue', employee.emp_fatigue)
            elif employee.emp_fatigue <= 0:
                self._add_issue(ValidationLevel.WARNING,
                              "Fatigue multiplier is non-positive",
                              'Employee', emp_id,
                              'emp_fatigue', employee.emp_fatigue)
    ...
    def _validate_task(self, task, task_id):
        ...
        if hasattr(task, 'task_name'):
            if not task.task_name:
                ...
                self._add_issue(ValidationLevel.ERROR, 
                              "Task name consists only of whitespace",
                              'Task', task_id, 
                              'task_name', task.task_name)
        ...
        if hasattr(task, 'task_crit'):
            if not isinstance(task.task_crit, int):
                ...
                self._add_issue(ValidationLevel.ERROR, 
                              "Criticality must be in range 1 to 5",
                              'Task', task_id, 
                              'task_crit', task.task_crit)
        ...
        if hasattr(task, 'task_duration_dist'):
            if not isinstance(task.task_duration_dist, (list, tuple)):
                ...
                self._add_issue(ValidationLevel.ERROR,
                              f"task_duration_dist must have 3 elements (got {len(task.task_duration_dist)})",
                              'Task', task_id, 
                              'task_duration_dist', task.task_duration_dist)
            else:
                ...
                if not all(isinstance(x, (int, float)) for x in task.task_duration_dist):
                    self._add_issue(ValidationLevel.ERROR, 
                                  "All duration values must be numbers",
                                  'Task', task_id, 
                                  'task_duration_dist', task.task_duration_dist)
                else:
                    # Validate order: optimistic ≤ likely ≤ pessimistic
                    if not (task.task_duration_dist[0] <= task.task_duration_dist[1] <= task.task_duration_dist[2]):
                        self._add_issue(ValidationLevel.ERROR,
                                      f"Durations must be in order: optimistic ≤ likely ≤ pessimistic (got {task.task_duration_dist})",
                                      'Task', task_id, 
                                      'task_duration_dist', task.task_duration_dist)
            ...
            if not isinstance(task.task_assigned_to, list):
                ...
                for emp_id in task.task_assigned_to:
                    if not isinstance(emp_id, int):
                        self._add_issue(ValidationLevel.ERROR,
                                      f"Assigned employee ID must be an integer: {emp_id}",
                                      'Task', task_id,
        ...
        if hasattr(task, 'task_primary_assignee'):
            if task.task_primary_assignee is not None:
                if not isinstance(task.task_primary_assignee, int):
                    self._add_issue(ValidationLevel.ERROR,
                                  "Primary assignee must be an integer (ID)",
                                  'Task', task_id,
    ...
    def _validate_dependencies(self, project):
        ...
        # Collect all task IDs for reference validation
        ...
        if isinstance(project.proj_dependencies, dict):
            ...
            self._add_issue(ValidationLevel.ERROR,
                            "proj_dependencies must be a list or dict",
                            'Project', 'project', 'proj_dependencies')
            return
    ...
    def _validate_dependency(self, dependency, dep_id, task_ids):
        ...
        # Validate dependency ID only if it exists (for dict case)
        ...
        if not isinstance(dependency.dep_from_task, int):
            self._add_issue(ValidationLevel.ERROR, 
                          "dep_from_task must be an integer", 
                          'Dependency', dep_id, 
                          'dep_from_task', dependency.dep_from_task)
        elif dependency.dep_from_task not in task_ids:
            self._add_issue(ValidationLevel.ERROR,
                          f"Predecessor task {dependency.dep_from_task} does not exist in project",
                          'Dependency', dep_id,
                          'dep_from_task', dependency.dep_from_task)
                
        # Validate successor task
        if not isinstance(dependency.dep_to_task, int):
            self._add_issue(ValidationLevel.ERROR, 
                          "dep_to_task must be an integer", 
                          'Dependency', dep_id, 
                          'dep_to_task', dependency.dep_to_task)
        elif dependency.dep_to_task not in task_ids:
            self._add_issue(ValidationLevel.ERROR,
                          f"Successor task {dependency.dep_to_task} does not exist in project",
                          'Dependency', dep_id,
                          'dep_to_task', dependency.dep_to_task)
        ...
        if not isinstance(dependency.dep_lag, (int, float)):
            ...
            self._add_issue(ValidationLevel.WARNING,
                          f"Negative lag ({dependency.dep_lag}) may cause problems",
                          'Dependency', dep_id,
                          'dep_lag', dependency.dep_lag)
    ...
    def _validate_outsource(self, outsource, outs_id):
        """Validates a single outsourcing option."""
        # Validate ID
        if not hasattr(outsource, 'outs_id'):
            self._add_issue(ValidationLevel.ERROR,
                          "Outsourcer has no ID",
                          'Outsource', outs_id)
        elif not isinstance(outsource.outs_id, int):
            self._add_issue(ValidationLevel.ERROR,
                          "Outsourcer ID must be an integer",
                          'Outsource', outs_id,
        ...
        if hasattr(outsource, 'outs_skills'):
            if not isinstance(outsource.outs_skills, list):
                self._add_issue(ValidationLevel.ERROR,
                              "Outsourcer skills must be a list",
                              'Outsource', outs_id,
        ...
        if hasattr(outsource, 'outs_lead_time_days'):
            if not isinstance(outsource.outs_lead_time_days, int):
                self._add_issue(ValidationLevel.ERROR,
                              "Lead time must be an integer (days)",
                              'Outsource', outs_id,
        ...
        if hasattr(outsource, 'outs_duration_multiplier'):
            if not isinstance(outsource.outs_duration_multiplier, (int, float)):
                ...
                self._add_issue(ValidationLevel.WARNING,
                              "Duration multiplier is less than 1.0 (outsourcer faster than internal team)",
                              'Outsource', outs_id,
                              'outs_duration_multiplier', outsource.outs_duration_multiplier)
    ...
    def print_report(self, report: dict):
        """Prints validation report in readable format."""
        print("=" * 60)
        ...
        if report["is_valid"]:
            ...
            print(f"Found errors: {report['errors']}")
        
        print(f"  Warnings: {report['warnings']}")
        print(f"  Info messages: {report['infos']}")
